...
Orchestrates: android_tv_guru.callsigns, .stations, .grouping.

EPG generation (android_tv_guru.epg) is a deliberately separate pipeline —
see docs/EPG_MATCHING.md — run as its own workflow step after playlists are
published, so the channel universe it matches against is current. It is not
called from here.
"""
...
# Self-hosted, generated by android_tv_guru.epg / .github/workflows/update-epg.yml.
# See docs/DATA_SOURCES.md for why the previous iptv-org.github.io guide URL
# was retired (it started returning HTTP 404) and docs/EPG_MATCHING.md for
# how this file is produced. This exact string must survive every playlist
# rebuild — see tests/test_playlists.py::test_epg_header_is_production_url.
EPG_HEADER = (
    '#EXTM3U x-tvg-url='
